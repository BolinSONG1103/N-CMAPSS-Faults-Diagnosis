...
def fig_robustness():
    ...
    ax.plot(n.amplitude,n.mean_stage_kappa,"^-",color=S.ACCENT,label="等级κ")
    ...
    ax.plot(b.amplitude,b.mean_stage_kappa,"s-",color=S.ACCENT,label="等级κ")
    if len(miss):
        ax.scatter([b.amplitude.max()+.18],[miss.mean_detection_F1.iloc[0]],marker="D",s=65,color="#756BB1",label="单通道缺失：检测F1")
        ax.scatter([b.amplitude.max()+.18],[miss.mean_stage_kappa.iloc[0]],marker="P",s=70,color="#31A354",label="单通道缺失：等级κ")
